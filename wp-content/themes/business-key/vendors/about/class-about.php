...
if ( ! class_exists( 'Business_Key_About' ) ) {
	...
	class Business_Key_About {
		...
		/**
		 * Singleton instance of Business_Key_About.
		 *
		 * @var Business_Key_About $instance Business_Key_About instance.
		 */
		private static $instance;

		/**
		 * Main Business_Key_About instance.
		 *
		 * @since 1.0.0
		 *
		...
		public static function init( $config ) {
			if ( ! isset( self::$instance ) && ! ( self::$instance instanceof Business_Key_About ) ) {
				self::$instance = new Business_Key_About;
		...
		public function setup_config() {
			...
			$dismiss_url = wp_nonce_url( add_query_arg( array( 'business-key-at-dismiss' => '1' ), admin_url( 'themes.php' ) ), 'business-key-action-dismiss-' . get_current_user_id(), 'business-key-dismiss-nonce' );
			$this->notification  = isset( $this->config['notification'] ) ? $this->config['notification'] : ( '<p>' . sprintf( esc_html__( 'Welcome! Thank you for choosing %1$s! To fully take advantage of the best our theme can offer please make sure you visit our %2$swelcome page%3$s.', 'business-key' ), $this->theme_name, '<a href="' . esc_url( admin_url( 'themes.php?page=' . $this->page_slug ) ) . '">', '</a>' ) . '</p><p><a href="' . esc_url( admin_url( 'themes.php?page=' . $this->page_slug ) ) . '" class="button button-primary" style="text-decoration: none;">' . sprintf( esc_html__( 'Get started with %s', 'business-key' ), $this->theme_name ) . '</a>' ) . '<a href="' . esc_url( $dismiss_url ) . '" class="btn-at-dismiss">' . esc_html__( 'Dismiss', 'business-key' ) . '</a></p>';
		}
		...
		function hide_notice() {
			// Verify nonce.
			if (! ( isset( $_GET['business-key-dismiss-nonce'] ) && wp_verify_nonce( sanitize_key( $_GET['business-key-dismiss-nonce'] ), 'business-key-action-dismiss-' . get_current_user_id() ) ) ) {
				return;
			}

			if ( 1 === absint( $_GET['business-key-at-dismiss'] ) ) {
				update_user_meta( get_current_user_id(), 'business_key_at_dismissed', 1 );
			}
		}
		...
		public function activation_admin_notice() {
			...
			$dismissed_status = get_user_meta( get_current_user_id(), 'business_key_at_dismissed', true );
		...
		public function load_assets( $hook ) {
			...
			if ( 'appearance_page_' . $this->page_slug === $hook ) {
				...
				wp_enqueue_style( 'business-key-about', get_template_directory_uri() . '/vendors/about/css/about' . $min . '.css', array(), '2.0.2' );
				wp_enqueue_script( 'business-key-about', get_template_directory_uri() . '/vendors/about/js/about' . $min . '.js', array( 'jquery' ), '2.0.2' );
				$js_vars = array(
					'ajaxurl' => esc_url( admin_url( 'admin-ajax.php' ) ),
				);
				wp_localize_script( 'business-key-about', 'businessKeyAboutObject', $js_vars );
			}
		}
						...
						if ( ! empty( $action_value['plugin_slug'] ) ) {
							...
							switch ( $active['needs'] ) {
								...
								case 'install':
									...
									$label = esc_html__( 'Install', 'business-key' );
									break;
								case 'activate':
									$class = 'activate-now button button-primary';
									$label = esc_html__( 'Activate', 'business-key' );
									break;
								case 'deactivate':
									$class = 'deactivate-now button';
									$label = esc_html__( 'Deactivate', 'business-key' );
									break;
							}
							?>
		...
		public function upgrade_to_pro() {
			...
			if ( isset( $upgrade_to_pro['button_link'] ) && ! empty( $upgrade_to_pro['button_link'] ) ) {
				$button_text = esc_html__( 'Upgrade to Pro', 'business-key' );
		...
		public function useful_plugins() {
			...
			if ( isset( $tgmpa ) ) {
				...
				if ( true === $complete ) {
					echo '<div class="all-active"><span class="dashicons dashicons-yes"></span>&nbsp;' . esc_html__( 'Great, all recommended plugins are intalled and active.', 'business-key' ) . '</div>';
				}
				...
				if ( true !== $complete ) {
					echo '<div class="button-wrap"><a href="' . esc_url( $url ) . '" class="button button-primary">' . esc_html__( 'Manage Plugins', 'business-key' ) . '</a></div>';
				}
